  ...
  Predictor: class {
    ...
    initPredictions() {
      ...
      const predictions = {
        ...
        'Pick Locks': { affected: ['gold'], effect: r => r.gold += parseInt($(g.goldCostLocks).text()) },
        'Buy Glasses': { effect: r => (r.gold -= 10, r.glasses = true) },
        'Buy Mana': { affected: ['mana', 'gold'], effect: r => (r.mana += r.gold * 50, r.gold = 0) },
        'Meet People': {},
        'Train Strength': {},
        'Short Quest': { affected: ['gold'], effect: r => r.gold += parseInt($(g.goldCostSQuests).text()) },
        'Investigate': {},
        'Long Quest': { affected: ['gold', 'rep'], effect: r => (r.gold += parseInt($(g.goldCostLQuests).text()), r.rep += 1) },
